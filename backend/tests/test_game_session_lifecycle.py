...
# --- Encapsulation: no live Scorecard escapes --------------------------


def test_game_session_has_no_public_live_scorecard_accessor():
    """Direct encapsulation regression. `GameSession` used to have a public
    `.scorecard` property handing out the live, mutable `Scorecard` — the
    exact escape hatch this module's docstring warns against. It must not
    exist: `current_snapshot()` and the `GameStateSnapshot` a throw()/reset()
    returns are the only public way to inspect a game's frames, score,
    completion state, next roll, and rack. This test never touches
    `.scorecard` itself (it can't — asserting its absence is the point) and
    never holds a live scorecard reference of its own.
    """
    session = GameService().create_game()
